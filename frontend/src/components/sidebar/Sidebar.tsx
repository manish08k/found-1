...
const NAV = [
  ...
  },
  {
    ...
    icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M3 3h18l-2 9H5L3 3z"/><path d="M3 3l-1-1"/><circle cx="9" cy="20" r="1.5"/><circle cx="17" cy="20" r="1.5"/></svg>
  },
  {
  ...
  {
    id: 'document-stores', label: 'Document Stores',
  ...
  },
  {
    ...
    icon: <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M4 7h16M4 12h16M4 17h16"/><circle cx="2" cy="7" r="1" fill="currentColor"/><circle cx="2" cy="12" r="1" fill="currentColor"/><circle cx="2" cy="17" r="1" fill="currentColor"/></svg>
  },
  {
...
export default function Sidebar() {
  ...
  return (
    <nav style={{ width: 52, background: 'var(--bg1)', borderRight: '1px solid var(--border)', display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '12px 0', gap: 4, flexShrink: 0, zIndex: 10 }}>
      ...
      {NAV.map(n => (
        <button key={n.id} title={n.label} aria-label={n.label} aria-current={page === n.id ? 'page' : undefined} onClick={() => handleNav(n.id)} style={{ width: 36, height: 36, borderRadius: 8, background: page === n.id ? 'var(--accent)' : 'transparent', color: page === n.id ? '#fff' : 'var(--text3)', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.15s', border: 'none', cursor: 'pointer' }}>
          {n.icon}
        </button>
      ))}
      <div style={{ flex: 1 }} />
      <button title="Logout" aria-label="Log out" onClick={logout} style={{ width: 36, height: 36, borderRadius: 8, background: 'transparent', color: 'var(--text3)', display: 'flex', alignItems: 'center', justifyContent: 'center', border: 'none', cursor: 'pointer' }}>
